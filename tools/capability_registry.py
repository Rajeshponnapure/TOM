...
CAPABILITIES: List[Dict[str, Any]] = [
    ...
    {"id": "file_ops", "label": "File ops: find/organize/rename/zip (approval-gated)",
     "category": "system", "triggers": ["organize my downloads folder by type",
     "find all pdfs in documents", "zip the folder downloads"], "surfaces": "chat+cli+gui",
     "executor": "tools/file_ops.py::FileOps.execute_plan"},
    {"id": "image_convert", "label": "Convert images between formats", "category": "system",
     "triggers": ["convert all images in pictures to png"], "surfaces": "chat+cli+gui",
     "executor": "tools/file_ops.py::FileOps.convert_images"},
    {"id": "web_scrape", "label": "Scrape web tables to CSV", "category": "web",
     "triggers": ["scrape the tables from URL"], "surfaces": "chat+cli+gui",
     "executor": "tools/web_recipes.py::scrape_tables"},
    {"id": "web_download", "label": "Download files from URLs (size-capped)", "category": "web",
     "triggers": ["download the file from URL"], "surfaces": "chat+cli+gui",
     "executor": "tools/web_recipes.py::download_file"},
    {"id": "code_run_node", "label": "Run Node.js files (approval-gated)", "category": "dev",
     "triggers": ["run app.js"], "surfaces": "chat+cli+gui",
     "executor": "tools/code_runner.py::CodeRunner.run_node_file"},
    {"id": "shell_run", "label": "Run shell commands (approval-gated, denylisted)", "category": "dev",
     "triggers": ["run shell: <command>"], "surfaces": "chat+cli+gui",
     "executor": "tools/code_runner.py::CodeRunner.run_shell"},
    {"id": "schedule_user", "label": "Schedule any task on a repeat interval", "category": "agents",
     "triggers": ["schedule: <task> every 2 hours", "list scheduled tasks"], "surfaces": "chat+cli+gui",
     "executor": "agent.py::schedule_user_task"},
